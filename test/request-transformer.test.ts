...
describe("Request Transformer Module", () => {
  describe("normalizeModel", () => {
    ...
    it('should normalize variants containing "codex" to gpt-5.1-codex', async () => {
      ...
      expect(normalizeModel("custom-gpt-5-codex-variant")).toBe(
        "gpt-5.1-codex"
      );
    });
  ...
  describe("transformRequestBody", () => {
    ...
    describe("Integration: Complete Config Scenarios", () => {
      ...
      describe("Variant Configuration (Modern Config Format)", () => {
        it("should use reasoning config from providerOptions when present", async () => {
          ...
          // ProviderOptions config should override default
          expect(result.reasoning?.effort).toBe("high");
          expect(result.reasoning?.summary).toBe("detailed");
        });

        it("should use textVerbosity from providerOptions when present", async () => {
          const body: RequestBody = {
            model: "gpt-5.2",
            providerOptions: {
              openai: {
        ...
        it("should fall back to plugin defaults when providerOptions not present", async () => {
          const body: RequestBody = {
            model: "gpt-5.2",
          };
          ...
          // Should use default reasoning effort for gpt-5.2 (high, as it supports xhigh)
          expect(result.reasoning?.effort).toBe("high");
          // Should use default summary
          expect(result.reasoning?.summary).toBe("auto");
          // Should use default text verbosity
          expect(result.text?.verbosity).toBe("medium");
        });

        it("should prioritize existing reasoning config (from AI SDK/variant) over userConfig", async () => {
          const userConfig: UserConfig = {
            global: { include: ["reasoning.encrypted_content"] },
            models: {},
          };

          // After opencode fix: AI SDK sets body.reasoning directly from variant selection
          const body: RequestBody = {
            model: "gpt-5.2",
            reasoning: {
              effort: "xhigh",
          ...
          // Existing reasoning config from AI SDK/variant should be respected
          expect(result.reasoning?.effort).toBe("xhigh");
          expect(result.reasoning?.summary).toBe("detailed");
          // text verbosity should still be updated from body
          expect(result.text?.verbosity).toBe("high");
        });

        it("should prioritize existing reasoning config (from AI SDK/variant) over per-model config", async () => {
          const userConfig: UserConfig = {
            global: {},
            models: {
              "gpt-5.2": {
          ...
          // Existing reasoning config from AI SDK should override per-model config
          expect(result.reasoning?.effort).toBe("high");
          // text verbosity should still respect per-model config
          expect(result.text?.verbosity).toBe("low");
        });

        it("should normalize existing reasoning effort for Codex Mini", async () => {
          // After opencode fix: AI SDK sets body.reasoning directly
          const body: RequestBody = {
            model: "gpt-5.1-codex-mini",
            reasoning: {
              effort: "none", // Not supported by Codex Mini, should upgrade to medium
            },
          };

          const result = await transformRequestBody(body, codexInstructions);

          // Existing "none" from variant should be normalized to "medium" for Codex Mini
          expect(result.reasoning?.effort).toBe("medium");
        });

        it("should respect existing xhigh reasoning for Codex Max", async () => {
          // After opencode fix: AI SDK sets body.reasoning directly
          const body: RequestBody = {
            model: "gpt-5.1-codex-max",
            reasoning: {
          ...
          // Codex Max supports xhigh
          expect(result.reasoning?.effort).toBe("xhigh");
          expect(result.reasoning?.summary).toBe("detailed");
        });

        it("should normalize xhigh to high for models that do not support it", async () => {
          // After opencode fix: AI SDK sets body.reasoning directly
          const body: RequestBody = {
            model: "gpt-5.1-codex",
            reasoning: {
              effort: "xhigh", // Not supported by gpt-5.1-codex, should downgrade to high
            },
          };

          const result = await transformRequestBody(body, codexInstructions);

          // gpt-5.1-codex doesn't support xhigh, should downgrade to high
          expect(result.reasoning?.effort).toBe("high");
        });

        it("should normalize none to low for Codex models", async () => {
          // After opencode fix: AI SDK sets body.reasoning directly
          const body: RequestBody = {
            model: "gpt-5.1-codex",
            reasoning: {
              effort: "none", // Not supported by Codex models, should upgrade to low
            },
          };

          const result = await transformRequestBody(body, codexInstructions);

          // Codex models don't support "none", should upgrade to "low"
          expect(result.reasoning?.effort).toBe("low");
        });

        it("should work with Ctrl+T variant cycling scenario", async () => {
          const userConfig: UserConfig = {
            global: {
              reasoningEffort: "medium",
              reasoningSummary: "auto",
          ...
          // After opencode fix: AI SDK sets body.reasoning directly from Ctrl+T variant selection
          const body: RequestBody = {
            model: "gpt-5.1-codex",
            reasoning: {
              effort: "high",
          ...
          // Variant config should override global config
          expect(result.reasoning?.effort).toBe("high");
          expect(result.reasoning?.summary).toBe("detailed");
          // text verbosity should still respect per-model config
          expect(result.text?.verbosity).toBe("medium"); // From global config
        });
        ...
        it("should normalize xhigh to high for models that do not support it", async () => {
          const body: RequestBody = {
            ...
            providerOptions: {
              openai: {
                reasoningEffort: "xhigh", // Not supported by gpt-5.1-codex
              },
            },
          };
        ...
        it("should normalize none to low for Codex models", async () => {
          const body: RequestBody = {
            ...
            providerOptions: {
              openai: {
                reasoningEffort: "none", // Not supported by Codex models
              },
            },
          };
        ...
        it("should handle partial providerOptions config", async () => {
          const body: RequestBody = {
            model: "gpt-5.2",
            providerOptions: {
              openai: {
                reasoningEffort: "high",
                // reasoningSummary and textVerbosity not provided
              },
            },
          };
          ...
          // Should use provided effort, but fall back for summary and verbosity
          expect(result.reasoning?.effort).toBe("high");
          expect(result.reasoning?.summary).toBe("auto"); // Default
          expect(result.text?.verbosity).toBe("medium"); // Default
        });

        it("should work with Ctrl+T variant cycling scenario", async () => {
          // Simulates what happens when user presses Ctrl+T to cycle from "medium" to "high"
          const userConfig: UserConfig = {
            global: {
              reasoningEffort: "medium",
              textVerbosity: "medium",
